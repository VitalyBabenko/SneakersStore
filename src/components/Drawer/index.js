  ...
  const onClickOrder = async () => {
    try {
      ...
      const { data } = await axios.post(
        `https://6273f31a345e1821b224d416.mockapi.io/orders`,
        {
          items: cartItems,
        }
      );
      ...
      for (let i = 0; i < cartItems.length; i++) {
        ...
        await axios.delete(
          `https://6273f31a345e1821b224d416.mockapi.io/cart/` + item.id
        );
        await delay(1000);
      }
    } catch (error) {
    ...
    >
      <div className={styles.drawer}>
        ...
        {items.length > 0 ? (
          ...
          <Info
            image={
              isOdrerComplite ? "img/complited-order.jpg" : "img/empty-cart.png"
            }
            title={isOdrerComplite ? "Заказ оформлен!" : "Корзина пустая"}
            discription={
              isOdrerComplite
                ? `Ваш заказ №${OdrerId} скоро будет передан курьерской доставке`
